    ...
    class Container {
        ...
        setupAccordionSidePanel() {
            ...
            $(".panel-group").on("click", ".close-panel", function () {
                $(".panel-group")
                    .find($(this).parents(".panel-default:first"))
                    .remove()
                $.when($(this).parents(".panel-default:first").remove()).then(
                    function () {
        ...
        setupPTPaging() {
            ...
            $("#page-submit").click(function () {
                container.startPage = parseInt(
                    $("#start-page").val().toString()
                )
            ...
            _.forEach(sectionList.sections, function (value, key) {
                if (
                    value.page >= container.startPage &&
                    value.page <= container.endPage
                ) {
        ...
        getSectionByLabel() {
            ...
            if (section) {
                ...
                let tlpCanvas = svgPanZoom("#tractatus-map")
                let mapSize = tlpCanvas.getSizes()
                //largest x_axis values is 129, largest y_axis value is 71
                let unitsHigh = mapSize.height / 71
                let unitsWide = mapSize.width / 129
                let xPos = unitsWide * section.x_axis
                let yPos = unitsHigh * section.y_axis
                console.log(mapSize)
                tlpCanvas.zoomAtPoint(4, {
                    x: Math.abs(xPos),
                    y: Math.abs(yPos),
                })
            }
        ...
        setupD3() {
            ...
            let elem = svg.selectAll("g").data(sectionAr)
            /* Define the data for the lines */
            let elemLine = svg.selectAll("g").data(lineAr)
            let elemLineEnter = elemLine.enter().append("g")
            /*Create and place the "blocks" containing the circle and the text */
            let elemEnter = elem.enter().append("g")
            let line = elemLineEnter
                .append("line")
                .attr("x1", function (d) {
                ...
                .attr("stroke", function (d) {
                    if (container.template == "pt") {
                        return d.checkLineColor(
                            container.pageFilteredPTList,
                            d.start,
                            d.end,
                            d.color
                        )
            ...
            let circle = elemEnter
                ...
                .attr("stroke", function (d) {
                    if (container.template == "pt") {
                        return d.checkCircleColor(
                            container._pageFilteredPTList,
                            d.label,
                            "black"
                        )
                    }
                ...
                .on("click", function (d) {
                    console.log("d", d)
                    container.divCounter = d.displayText(
                        true,
                        container.version,
                        container.divCounter,
                        container.template,
